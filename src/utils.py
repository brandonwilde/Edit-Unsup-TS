...
class Lang:
    ...
    def readLangsSingle(dataset, split):
        print("Reading lines...")
        data[split + "_src"] = []
        data[split + "_dst"] = []
        if dataset == 'Wikilarge':
            print('loading Wikilarge data')
            with open(f"../data-simplification/wikilarge/wiki.full.aner.ori.{split}.src") as file:
                data[split + "_src"].append(file.read())
            with open(f"../data-simplification/wikilarge/wiki.full.aner.ori.{split}.dst") as file:
                data[split + "_dst"].append(file.read())
        
        valid_simple, valid_simple_unique = Lang.getSentences(data[split + "_dst"], config['lm_backward'])
...
def prepareEvalData(embedding_dim, freq, ver, dataset, operation):
    valid_simple, valid_simple_unique, valid_complex, valid_complex_unique, output_lang, tag_lang, dep_lang = Lang.readLangs(dataset)
...
def prepareData(embedding_dim, freq, ver, dataset, operation):
    ...
    # vocab is always made up from all the sentences and not unique since we have a min frequency clause
